...
The database needs to be generated before starting the containers. This downloads ~1 GB of compressed data from IMDB and takes 15–30 minutes.
...
Open `http://localhost` in your browser. The frontend waits for the backend to be healthy before starting — no manual ordering needed.
...
├── Dockerfile.frontend       # nginx image serving the built React app
├── nginx.conf                # Serves the SPA and proxies /api to the backend
├── backend/
│   ├── Dockerfile.backend    # Python image running uvicorn
